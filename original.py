...
class Neuron:
    ...
    def getOutput(self):
        self.output = sigmoid(self.input_sum) #入力データの合計値をシグモイド関数で加工したものを出力
        return self.output
...
class NeuralNetwork:
    # 重み
    w_im = [[0.496, 0.512], [-0.501, 0.998], [0.498, -0.502]] #入力層から中間層
    w_mo = [0.121, -0.4996, 0.200] #中間層から出力層

    # 各層の宣言
    input_layer = [0.0, 0.0, 1.0]#データ、データ、バイアス
